...
export default function RouteShipments() {
  ...
  useEffect(() => {
    ...
    const VeifyInitRoute = async () => {
      ...
      if (status_init && status_init.comment && route.type === "shipment") {
        navigate("/route/road");
      }

      if (status_init && status_init.comment && route.type === "pickup") {
  ...
  return (
    <div className="">
      ...
      <div className="m-2 flex flex-col items-center w-fit justify-center">
        <img src={isologokarggu} alt="Img karggu" width={50} height={150} />
      ...
      <div className="shadow-2xl rounded-xl p-5 m-5">
        ...
        <p>
          <span className="text-red-500 font-medium">Operador:</span> {route.operator}
        </p>
        <p>
          <span className="text-red-500 font-medium">Vehículo:</span> {vehicle.model}
        </p>
        <p>
          <span className="text-red-500 font-medium">Placas:</span> {vehicle.plates}
        </p>
        {milleage.regist ? (
          <p>
            <span className="text-red-500 font-medium">Kilometraje:</span> {milleage.milleage} Km
          </p>
        ) : (
          <p>
            <span className="text-red-500 font-medium">Kilometraje:</span>{" "}
            <input
              onChange={(e) => setMilleage({ ...milleage, milleage: e.target.value, error: false })}
              className="border-solid border-2 border-red-500 rounded-md px-3 w-2/6"
              type="number"
              placeholder="0 km"
            />
            <button className="bg-sky-400 ml-1 font-bold rounded-md px-1 py-1 text-white" onClick={handleMilleage}>
              Registrar
            </button>
          </p>
        )}
        ...
        <div>
          {route.type === "shipment" ? (
            ...
            <p className="text-red-500 font-medium">Envíos a recolectar:</p>
          )}
          {route.type === "pickup" ? (
            <ul>
              {shipments &&
                shipments.map((shipment, i) => (
                  <li key={i} className="p-2 px-4 my-2 bg-blue-500 rounded-full shadow-xl text-white font-bold">
                    {i + 1}. {shipment._id}{" "}
                    <p>
                      Dirección: {shipment.address.city} {shipment.address.state} {shipment.address.colony}{" "}
                      {shipment.address.street} {shipment.address.zipCode}
                    </p>
                  </li>
                ))}
            </ul>
          ...
          {route.type === "shipment" ? (
            <ul>
              {shipments.map((shipment, i) => (
                <li
                  key={i}
                  className={`p-2 px-4 my-2 rounded-full shadow-xl text-white font-bold ${
                    shipment.stateHistory.find((state) => state.comment === "Cargado al camión") ? "bg-green-500" : "bg-red-500"
                  }`}>
                  {i + 1}. {shipment._id}{" "}
                  {shipment.stateHistory.find((state) => state.comment === "Cargado al camión") ? "Registrado" : "No Registrado"}
                </li>
              ))}
            </ul>
          ) : null}
        ...
        <button
          onClick={handleInitRoute}
          className="mt-5 p-2 text-white font-bold bg-gray-400 rounded-lg hover:bg-white hover:text-green-600 shadow-lg">
          Iniciar ruta
        </button>
      </div>
    </div>
